    ...
    txQueue = txQueue.then(async () => {
      try {
        ...
        if (err.code === "NONCE_EXPIRED" || (err.message && err.message.includes("nonce"))) {
          console.warn("⚠️  Nonce error — retrying...");
          try {
...
async function buildVoteMerkleTree() {
  const allVotes = await Voter.find({ phase: "revealed" }).select("commitmentHash");
  if (allVotes.length === 0) return { tree: null, root: null, leaves: [] };
  const leaves = allVotes.map((v) => Buffer.from(v.commitmentHash.replace(/^0x/, ""), "hex"));
...
function generateNonce() { return "0x" + crypto.randomBytes(32).toString("hex"); }
function computeCommitmentHash(didHash, candidateId, nonce) {
  return ethers.keccak256(ethers.solidityPacked(["bytes32", "uint256", "bytes32"], [didHash, candidateId, nonce]));
}
...
const IP_RISK_INCREMENT = 15;

async function checkSuspiciousIP(student, req) {
  const ip = req.ip;
  if (!ip) return { flagged: false };
  const windowStart = new Date(Date.now() - IP_WINDOW_MS);
  const ipCount = await FraudLog.countDocuments({ ipAddress: ip, createdAt: { $gte: windowStart } });
  if (ipCount > IP_FRAUD_THRESHOLD) {
    const reason = `Suspicious IP activity — ${ipCount} requests from ${ip} in the last hour (threshold: ${IP_FRAUD_THRESHOLD})`;
    student.riskScore = (student.riskScore || 0) + IP_RISK_INCREMENT;
    if (!student.suspiciousIPs.includes(ip)) student.suspiciousIPs.push(ip);
    await student.save();
    await FraudLog.create({ student: student._id, studentHash: student.studentHash, reason, ipAddress: ip, severity: "high" });
    console.warn(`🚨 Suspicious IP flagged: ${ip} — student ${student.studentId} — count: ${ipCount}`);
    return { flagged: true, ipCount, reason };
  }
  return { flagged: false, ipCount };
}
...
async function handleFraud(student, reason, req, reportToChain = false) {
  ...
  if (student.failedAttempts >= 3) {
    ...
    try {
      ...
      await sendTx((nonce) => contract.blacklistVoter(didHash, reason, { nonce }));
    } catch (chainErr) { console.error("Blockchain blacklist error:", chainErr.message); }
  }
  await student.save();
  await FraudLog.create({ student: student._id, studentHash: student.studentHash, reason, ipAddress: req.ip, severity: severityLevel });
  if (reportToChain) {
    try {
      const didHash = generateDIDHash(student.studentId);
      await sendTx((nonce) => contract.reportFraud(didHash, reason, { nonce }));
    } catch (chainErr) { console.error("Blockchain fraud report error:", chainErr.message); }
  }
}
...
function authenticateStudent(req, res, next) {
  ...
  try { req.student = jwt.verify(token, process.env.JWT_SECRET); next(); }
...
router.get("/status", authenticateStudent, async (req, res) => {
  try {
    ...
    const isEffectivelyRegistered = student.isDIDRegistered === true || (student.status === "approved" && !!student.didHash);
    if (!student.isDIDRegistered && isEffectivelyRegistered) { student.isDIDRegistered = true; await student.save(); }
    let statusData = { isRegistered: isEffectivelyRegistered, isEligible: student.isEligible || false, hasVoted: false, isBlacklisted: student.isBlacklisted || false, fraudScore: student.riskScore || 0, didHash: student.didHash || null };
    const activeElection = await Election.findOne({ status: "active" });
    if (activeElection) {
      const voterRecord = await Voter.findOne({ student: student._id, election: activeElection._id, phase: "revealed" });
      statusData.hasVoted = !!voterRecord;
    }
    if (student.didHash) {
      try {
        const [chainRegistered, chainEligible, chainVoted, chainBlacklisted, chainFraudScore] = await contract.getVoterStatus(student.didHash);
      ...
      } catch (chainErr) { console.log("Blockchain status unavailable, using DB:", chainErr.message); }
    }
...
router.post("/register-did", async (req, res) => {
  try {
    ...
    try {
      ...
      if (isRegistered) {
        if (!student.isDIDRegistered) { student.isDIDRegistered = true; student.didHash = didHash; await student.save(); }
        return res.json({ success: true, message: "DID already registered", didHash });
      }
    } catch {}
    const tx = await sendTx((nonce) => contract.registerVoterDID(didHash, { nonce }));
    await tx.wait();
    student.didHash = didHash; student.isDIDRegistered = true; student.didRegisteredAt = new Date();
    await student.save();
    res.json({ success: true, message: "Voter DID registered on blockchain", didHash, transactionHash: tx.hash });
  } catch (error) { console.error("DID Registration Error:", error); res.status(500).json({ success: false, message: error.message }); }
});
...
router.post("/set-eligibility", async (req, res) => {
  try {
    ...
    if (!studentId || eligible === undefined) return res.status(400).json({ success: false, message: "studentId and eligible required" });
    const didHash = generateDIDHash(studentId);
    const tx = await sendTx((nonce) => contract.setVoterEligibility(didHash, eligible, { nonce }));
    await tx.wait();
...
router.post("/commit-vote", authenticateStudent, async (req, res) => {
  try {
    ...
    if (rapidCheck.flagged) {
      ...
      if (refreshed?.isBlacklisted) {
        return res.status(403).json({ success: false, message: "Too many rapid attempts — you have been blocked." });
      }
    }
    ...
    const activeElection = await Election.findOne({ status: "active" });
    if (!activeElection || !activeElection.isOpen) return res.status(403).json({ success: false, message: "Election is closed" });
    if (!student.isEligible) return res.status(403).json({ success: false, message: "You are not eligible to vote" });
    const candidateDoc = await Candidate.findById(candidateId);
    if (!candidateDoc || candidateDoc.status !== "approved") return res.status(400).json({ success: false, message: "Invalid or unapproved candidate" });
    ...
    const didHash = generateDIDHash(student.studentId);
    if (!student.didHash) { student.didHash = didHash; await student.save(); }
    ...
    try {
      ...
      if (!isRegisteredOnChain) {
        ...
        const regTx = await sendTx((nonce) => contract.registerVoterDID(didHash, { nonce }));
        await regTx.wait();
      ...
      if (!isEligibleOnChain) {
        ...
        const eligTx = await sendTx((nonce) => contract.setVoterEligibility(didHash, true, { nonce }));
        await eligTx.wait();
    ...
    } catch (chainErr) { console.log("Blockchain pre-check failed, continuing with DB fallback:", chainErr.message); }
    ...
    let blockNumber = 0;
    try {
      const tx = await sendTx((txNonce) => contract.commitVote(didHash, commitmentHash, { nonce: txNonce }));
      const receipt = await tx.wait();
      commitTxHash = tx.hash; blockNumber = receipt.blockNumber;
    } catch (chainErr) { console.log("Blockchain commit failed, using DB fallback:", chainErr.message); }

    await Voter.create({ election: activeElection._id, student: student._id, candidate: candidateDoc._id, didHash, studentHash: didHash, blockchainCandidateId, commitmentHash, nonce, commitTransactionHash: commitTxHash, phase: "committed" });
...
router.post("/reveal-vote", authenticateStudent, async (req, res) => {
  try {
    ...
    const activeElection = await Election.findOne({ status: "active" });
    if (!activeElection || !activeElection.isOpen) return res.status(403).json({ success: false, message: "Election is closed" });
    const voterRecord = await Voter.findOne({ election: activeElection._id, student: student._id, phase: "committed" });
    if (!voterRecord) return res.status(400).json({ success: false, message: "No commitment found. Please commit first." });
    ...
    if (candidateDoc) { candidateDoc.votes = await Voter.countDocuments({ candidate: candidateDoc._id, phase: "revealed" }); await candidateDoc.save(); }
...
router.post("/verify-my-vote", authenticateStudent, async (req, res) => {
  try {
    ...
    if (!verificationCode) return res.status(400).json({ success: false, message: "verificationCode required" });
    const student = await Student.findById(req.student.id);
    if (!student) return res.status(404).json({ success: false, message: "Student not found" });
    const voterRecord = await Voter.findOne({ student: student._id, verificationCode, phase: "revealed" }).populate("candidate");
    if (!voterRecord) return res.status(400).json({ success: false, message: "Vote record not found for this code" });
    let merkleProof = null; let merkleRoot = null; let merkleVerified = false;
    try {
      const { tree, root } = await buildVoteMerkleTree();
      if (tree && root) {
        merkleRoot = root;
        const leafBuffer = Buffer.from(voterRecord.commitmentHash.replace(/^0x/, ""), "hex");
...
router.get("/verify-receipt/:code", async (req, res) => {
  try {
    ...
    if (voterRecord) return res.json({ success: true, receiptExists: true, message: "Receipt is valid." });
    try { const exists = await contract.verifyReceiptExists(code); return res.json({ success: true, receiptExists: exists }); }
    catch { return res.json({ success: true, receiptExists: false }); }
...
router.get("/merkle-root", async (req, res) => {
  try {
    const { tree, root, leaves } = await buildVoteMerkleTree();
    if (!tree || leaves.length === 0) return res.json({ success: true, message: "No votes cast yet — Merkle tree is empty", root: null, totalVotes: 0, onChainRoot: null, isInSync: false });
    let onChainRoot = null; let onChainAnchorCount = 0; let onChainLastAnchorBlock = 0; let isInSync = false;
    try { onChainRoot = await contract.latestOffChainDataRoot(); onChainAnchorCount = Number(await contract.anchorCount()); onChainLastAnchorBlock = Number(await contract.lastAnchorBlock()); isInSync = onChainRoot === root; }
  ...
  } catch (error) { console.error("Merkle root error:", error); res.status(500).json({ success: false, message: error.message }); }
});

/* ── GET /merkle-proof/:commitmentHash ── */
router.get("/merkle-proof/:commitmentHash", async (req, res) => {
  try {
    const { commitmentHash } = req.params;
    if (!commitmentHash || !commitmentHash.startsWith("0x")) return res.status(400).json({ success: false, message: "Valid commitmentHash (0x...) required" });
    const voterRecord = await Voter.findOne({ commitmentHash, phase: "revealed" });
    if (!voterRecord) return res.status(404).json({ success: false, message: "Commitment hash not found in revealed votes" });
    const { tree, root, leaves } = await buildVoteMerkleTree();
    if (!tree) return res.status(400).json({ success: false, message: "No votes in tree yet" });
    const leafBuffer = Buffer.from(commitmentHash.replace(/^0x/, ""), "hex");
    const proof = tree.getHexProof(leafBuffer);
    const isValidLocally = tree.verify(proof, leafBuffer, tree.getRoot());
    let isVerifiedOnChain = false;
    try { const proofBytes32 = proof.map((p) => ethers.zeroPadValue(p, 32)); isVerifiedOnChain = await contract.verifyOffChainRecord(commitmentHash, proofBytes32); }
...
router.get("/results", async (req, res) => {
  try {
    const election = await Election.findOne({ status: { $in: ["completed", "active"] } }).sort({ updatedAt: -1 });
    if (!election) return res.status(400).json({ message: "No election available" });
    ...
    const results = await Promise.all(candidates.map(async (candidate) => {
      ...
      return { id: candidate._id, name: candidate.name, votes: voteCount, percentage: totalVotes > 0 ? ((voteCount / totalVotes) * 100).toFixed(2) : "0.00", photo: candidate.photo };
    }));
    results.sort((a, b) => b.votes - a.votes);
    let merkleRoot = null;
    try { const { root } = await buildVoteMerkleTree(); merkleRoot = root; } catch {}
    res.json({ electionTitle: election.title || "University Election", totalVotes, winner: results[0] || null, results, merkleRoot });
...
router.get("/winner", async (req, res) => {
  try {
    const election = await Election.findOne({ status: { $in: ["completed", "active"] } }).sort({ updatedAt: -1 });
    if (!election) return res.status(404).json({ success: false, message: "No election found" });
    const candidates = await Candidate.find({ election: election._id, status: "approved" });
    if (!candidates.length) return res.status(404).json({ success: false, message: "No candidates found" });
